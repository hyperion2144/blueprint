...
`specwf change new <name>` 每次创建 adhoc change 时，`updateState` → `saveState` 会调用 `generateStateBody()` 从零生成 Markdown body，覆盖用户在 `state.md` body 中写的所有详细内容。
...
`try/catch` 处理新文件场景（`specwf init` 时文件不存在，抛出异常，回退到默认 body）。

---

## Must-haves

- `specwf change new <name>` 创建 adhoc change 后，state.md body **不变**（只 frontmatter 追加 adhoc 条目）
- `specwf archive <change>` 归档后，state.md body 保留
- `specwf state set-milestone / set-phase / set-step` 后，state.md body 保留
- `specwf init` 创建新 state.md 时，body 正常生成（默认 body）
- 现有测试全部通过
