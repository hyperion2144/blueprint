...
<!--
  Orchestrator instructions for `bp archive [name]`.
  This command merges delta specs into the global specs, moves the change to archive/,
  and updates the roadmap progress.
-->
...
3. Apply deltas:
   ...
   - **MODIFIED Requirements**: Find the matching `### Requirement:` header in global spec, replace the entire requirement block (header + description + scenarios) with the new version
...
4. If all changes in the phase are `[x]`:
   - Change phase status to `[COMPLETED]`
5. If all phases in the milestone are `[COMPLETED]`:
   - Change milestone status to `[SHIPPED]`
...
✓ Archived $1
  ...
  {{If phase completed: "Phase {{phase-name}} COMPLETED"}}
  {{If milestone shipped: "Milestone {{milestone-name}} SHIPPED"}}
...
- **Delta spec merge is the critical operation.** If a MODIFIED requirement header doesn't match the global spec, flag it - don't silently skip.
