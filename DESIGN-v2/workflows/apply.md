...
<!--
  Orchestrator instructions for `bp apply [name]`.
  This command dispatches executor sub-agents per wave for parallel implementation.
  The orchestrator does NOT write code - executors do.
-->
...
2. Dispatch ALL ready waves CONCURRENTLY (one task tool call per wave, all in one batch)
   ...
   - Fresh context: yes
   - Isolated: true (if platform supports) — executor is the ONLY role that needs isolation: multiple executors run concurrently per wave, each modifying source files and making git commits. Planner and reviewer don't need isolation (single instance, only write to change directory).
