...
Blueprint (`@hyperion2144/blueprint` v2) is a **spec-driven development workflow CLI for AI coding agents**. It structures software changes into a 2-layer architecture (Roadmap + Change) with artifact-based progress detection, YAML schema validation, and fresh-context sub-agents (planner, executor, reviewer, codebase-scanner, refactorer, fixer).
...
| `src/templates/agents/` | Sub-agent system prompts (planner, executor, reviewer, codebase-scanner, refactorer, fixer) |
...
- 6 agents: planner (design + tasks + delta specs), executor (TDD waves), reviewer (triple review), codebase-scanner (brownfield bootstrap), refactorer (behavior-preserving consolidation), fixer (review-issue repair)
...
| `src/templates/agents/index.ts` | All 6 sub-agent system prompts (planner, executor, reviewer, codebase-scanner, refactorer, fixer) |
...
- `session_start` emits a `<bp-context>...</bp-context>` custom message. The
  block is a paths-only compact payload; sub-agent discrimination appends
  `## Roadmap State` (planner), inline `context.jsonl` rows with a
  `> GUARD-RAIL:` prefix (executor / fixer), `## Invariants` + `tasks.md`
  acceptance (reviewer), or `## Refactor Targets` (refactorer).
...
- Sub-agents (planner/executor/reviewer/codebase-scanner/refactorer/fixer) **never call** `bp context <step>`
