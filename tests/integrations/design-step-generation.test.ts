...
describe('design-step platform generation (T-5)', () => {
  ...
  it('generates 16 claude-code and agent skill/command files including the design steps', () => {
  ...
  it('generates each design-step body byte-identical to the registry instructions', () => {
    ...
    for (const step of DESIGN_STEPS) {
      ...
      expect(ompSkillFiles.find((f) => f.path === `.omp/skills/bp-${step}/SKILL.md`)!.content.endsWith(skillInstr)).toBe(true);
  ...
  it('keeps every new generated description free of colon+space', () => {
    ...
    for (const def of [...PI_SKILL_DEFS, ...CODEX_SKILL_DEFS, ...OMP_SKILL_DEFS, ...OMP_COMMAND_DEFS]) {
      if (DESIGN_STEPS.includes(def.step as (typeof DESIGN_STEPS)[number])) newDescriptions.push(def.description);
    }
    expect(newDescriptions).toHaveLength(20); // 5 pi + 5 codex + 5 omp skills + 5 omp commands
