...
Implement the non-behavior tasks yourself, one by one — after each task run its tests and commit, then mark it \`[x]\` with the commit hash in tasks.md (the marking edits are committed once at the end, not per task). After all tasks, run the full test suite and mark the Pre-Archive Checklist as in Step 4.
