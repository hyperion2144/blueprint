...
> These are the steps you (orchestrator) execute in order. \`bp archive\` only outputs these steps — it does not auto-execute. Actual archive is done by \`bp finish\`.
...
The change's delta specs must match what was actually implemented before they are merged into \`bp/specs/\`. Read \`bp/changes/$1/proposal.md\` (PR-N behaviors), \`bp/changes/$1/design.md\` (DS-N), the implementation (source + tests), and each delta spec at \`bp/changes/$1/specs/<domain>/spec.md\`:
- For every implemented behavior the delta spec does not yet require, ADD a requirement (with a scenario) to that domain's delta spec.
- For every delta requirement whose described behavior drifted from the implementation, MODIFY it — write the full new requirement text and annotate the change with a \`(was: ...)\` note.
- Write ONLY to the change's \`specs/<domain>/spec.md\` delta files — never to \`bp/specs/\` directly. The merge into \`bp/specs/\` happens when \`bp finish\` runs.

### Step 4: Run finish command

The \`bp finish\` command handles everything: review verification, delta spec merge, change directory move, and roadmap update.
...
- **Archive-check writes only to the change's delta specs.** Never edit \`bp/specs/\` directly — \`bp finish\` performs the merge.
- **If merge conflict occurs**, resolve in the delta spec (change directory) and re-run \`bp finish $1\`. Do NOT edit global specs directly.
