...
Check that \`bp/.codebase-map.md\` exists. It was auto-generated by the \`bp init\` command with module structure, public API, and dependency info. If missing, run \`bp map refresh\`.

The planner sub-agent reads this map first (before deep-diving source files) during \`bp plan\`. This is the entry point for context optimization — planner gets a structure overview instead of reading every source file.
