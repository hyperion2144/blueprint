...
**Key Decisions** — As the user discusses technical details (field design, API format, conventions, tech stack choices), write the decisions into each phase's \`### Key Decisions\` section. This prevents "we agreed on this in the kickoff" issues later. Each decision includes: subject area, decision content, reason, and alternatives considered.
