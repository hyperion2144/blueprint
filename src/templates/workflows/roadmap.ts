...
2. **Milestone/phase skeleton** — what milestone/phase structure does the user want?
...
- Do NOT proceed to Step 2 until direction and the milestone/phase skeleton are agreed without guessing.
...
**Milestones = product releases. Default: exactly ONE milestone covering the entire project.**
Do NOT create extra milestones to organize work — that's what phases are for.

**Only create additional milestones (M2+) for work the user EXPLICITLY mentioned wanting to do
later but that is NOT committed yet** — a vague future intention ("we'll probably do X someday").

- If all discussed work belongs to the same product release → 1 milestone, phases inside it.
- If the user explicitly said "later / someday / v2 idea" about something NOT in current scope
  → create it as a placeholder milestone (Goal/What only, NO phase decomposition).
- NEVER create a milestone just because the project is big or has many phases.
- NEVER create a milestone for work the user never mentioned.
...
- **Future-intention milestones** (placeholder only) — user explicitly mentioned wanting this later, but NOT discussed yet:
  - Goal / What (TBD is OK; fill what's known)
...
A real project MAY have one full-structure milestone (M1) plus one or more placeholder milestones
(M2 future) — but the default is a single milestone. Assign each milestone the tier that matches
its actual discussion state.
...
- **Phase contains MULTIPLE changes.** A phase is a slice of work with several features inside it —
  one phase = one change is wrong. If a phase has only one change, that's an over-split signal:
  merge that phase into a neighboring phase instead of keeping a phase per feature.
- **Change** = one feature. One feature corresponds to one change — do NOT split by task count or
  ...
  and by module/deliverable units into tasks (default 1 wave).
...
- Milestone count: default 1 — only add M2+ for user-expressed future intentions
- Every full-structure phase contains MULTIPLE changes; a phase with a single change is an over-split signal (merge into a neighbor phase)
...
- **Milestones = product releases. Default: exactly ONE milestone.** No "foundation", "setup", "scaffolding" — M1 = first shippable increment.
- **Only add M2+ for user-expressed future intentions** ("later/someday/v2 idea"). Never split milestones to organize phases.
- **A phase contains MULTIPLE changes.** One change per phase = over-split, merge into a neighbor phase.
