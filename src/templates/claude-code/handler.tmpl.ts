/**
 * claude-code/handler.tmpl.ts — string-constant source of truth for the
 * Claude Code hook handler script.
 *
 * The string exported as `HANDLER_SOURCE` is the full self-contained
 * `.mjs` source for the generated `.claude/hooks/bp-claude-handler.mjs`
 * file. `src/integrations/claude-code/handler.ts` re-exports this constant
 * and surfaces the testable handlers (isDisabled, hasBpConfig, etc.).
 *
 ...
 * are guaranteed identical and two calls to generateClaudeHandler()
 * with the same config return identical content.
 *
 * D-2: This is an independent near-duplicate of
