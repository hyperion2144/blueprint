/**
 * pi extension template — string-constant source of truth.
 *
 * The string exported as `EXTENSION_SOURCE` is the full self-contained
 * TypeScript source of the generated `.pi/extensions/bp/index.ts` file.
 * `src/integrations/pi/extension.ts` reads this constant (via the runtime
 * re-export) and emits it verbatim — no inline string literal duplicates
 * this content.
 *
 * WARNING: This file contains inline TypeScript that mirrors the logic in
 * src/integrations/pi/extension-runtime.ts. If you change logic here, you
 * MUST update extension-runtime.ts too (and vice versa).
 *
 ...
 *
 * Determinism: this module must stay byte-deterministic — no Date.now(),
 * no Math.random(), no env lookups at module load.
 */
