...
4. **Tasks are cohesive units** - Each task implements one module or deliverable unit with a RED test. If you can't write a failing test for it, it's not a good task. Do NOT split a module into one task per behavior path.
...
- **Codebase map queries** (run on demand — do NOT read .codebase-map.md directly):
  - \`bp map list\` — all modules with one-line responsibility (start here for overview)
  - \`bp map module <name>\` — full module detail (exports, depends_on, files)
  - \`bp map files <module>\` — list files in a module with export/import counts
  - \`bp map file <path>\` — full file detail (exports, imports, which module)
  - \`bp map symbol <name>\` — find which file defines a function/class/symbol
  - \`bp map impact <module>\` — who depends on this module (for change impact analysis)
...
1. **Codebase structure** — Run \`bp map list\` and \`bp map module <name>\` for module overview. Run \`bp map impact <module>\` to see who depends on modules you are changing.
...
1. Run \`bp map list\` for module overview
2. Run \`bp map module <name>\` for modules you will modify
3. Run \`bp map impact <module>\` to see who depends on the modules you are changing

Write \`## Impact Analysis\` section in design.md listing:
- **Direct Impacts**: files being modified and what changes
- **Indirect Impacts**: callers/dependents that may need updates (from \`bp map impact\`)
...
**Every DS-N must carry three contract fields**: \`Requirements\` (observable behaviors it must satisfy), \`Constraints\` (hard limits — protocols, boundaries, non-negotiables), and \`Acceptance Criteria\` (binary pass/fail — "X happens when Y"). Fill all three for every DS-N.
...
Each task (T-N) is **one cohesive module or deliverable unit** — a file, a module,
or a self-contained capability that can be implemented and committed as a unit.

**Examples (module granularity — one task per module):**
\`\`\`
- [ ] T-1: [type:behavior] ThemeContext (provide + toggle current theme) <!-- commit: -->
- [ ] T-2: [type:behavior] ThemeToggle (render current theme + toggle on click) <!-- commit: -->
- [ ] T-3: [type:scaffolding] Create ThemeToggle component shell <!-- commit: -->
\`\`\`

**Good decomposition (module-level):**
\`\`\`
- [ ] T-1: [type:behavior] ThemeContext — read/write/persist theme state <!-- commit: -->
- [ ] T-2: [type:behavior] ThemeToggle — render + click handling <!-- commit: -->
\`\`\`

**Bad decomposition (behavior-path level — over-split):**
\`\`\`
- [ ] T-1: [type:behavior] ThemeContext provides current theme <!-- commit: -->
- [ ] T-2: [type:behavior] ThemeContext toggles theme on call <!-- commit: -->
- [ ] T-3: [type:behavior] ThemeContext persists theme <!-- commit: -->
- [ ] T-4: [type:behavior] ThemeToggle renders current theme <!-- commit: -->
- [ ] T-5: [type:behavior] ThemeToggle calls toggle on click <!-- commit: -->
\`\`\`

**Rules:**
- **One task per DS-N by default.** A DS spanning multiple files is still ONE task if the files are implemented together (same test file, same commit).
- Only split a DS into multiple tasks when it has clearly separable layers that need independent testing (e.g. data model vs UI).
- Do NOT split by behavior path: multiple behaviors of one module belong in ONE task, with a RED test covering all of them.
- A task's RED test can contain multiple assertions/scenarios — one test file per task is the norm.
- **Granularity check: if a change has more than 15 tasks, you are over-splitting — merge tasks.** More than 5 tasks per wave is a signal too.
...
**HARD RULE: Every Requirement MUST have at least one \`#### Scenario:\` block. A Requirement with only a SHALL/MUST sentence and no Scenario block is INVALID. Scenarios MUST use the bolded keyword format (\`**GIVEN**\`, \`**WHEN**\`, \`**THEN**\`, \`**AND**\`) exactly as shown in the spec template — plain unbolded keywords are not accepted.**
...
- **Implementability self-check** (CRITICAL - the #1 reason designs get rejected by the orchestrator): For each DS-N, re-read your own Detailed Design and ask: "Can an executor implement this without reading source code or guessing?" Before returning, verify each of these is present:
  ...
  - Requirements, Constraints, and Acceptance Criteria present for every DS-N (acceptance criteria must be binary pass/fail)
...
- **Codebase map queries** (run on demand):
  ...
  - \`bp map impact <module>\` — who depends on this module (what your changes affect)
...
\`\`\`
Do NOT consolidate issues — one \`- [ ]\` line per finding.
...
**Verdict format - PLAIN TEXT ONLY:** Write the verdict word as plain text (e.g. \`## Overall Verdict: PASS\`). Do NOT wrap it in \`**\` (markdown bold) like \`**PASS**\`. The archive validator parses the verdict via regex and \`**PASS**\` will NOT match, blocking archiving. This applies to Overall Verdict, Spec Verdict, Goal Verdict, and the Constraint Checklist PASS/FAIL column.
