...
1. Read proposal.md → extract PR-1, PR-2 from ## Deliverables
   Note each PR's title and refs to understand scope
2. Decompose the solution into design items (modules/components):
   - Assign DS-1, DS-2, DS-3... sequentially
   - For each DS item: \`refs: PR-{id}, PR-{id}\` — which deliverables it implements
   - Every PR must be referenced by at least one DS (no orphan PRs — validation checks)
   - Write a concise description of each component's responsibilities and interfaces
3. Write Architecture section (component diagram, data flow, module boundaries)
4. Compare at least 2 alternatives in Alternatives section
5. Write the completed design to \`design.md\`
...
2. Group tasks into waves by theme. Name each wave meaningfully.
3. For each task:
   - Assign T-1, T-2... sequentially
   - \`refs: DS-{id}\` (required) — which design item this task implements
     Every DS must be referenced by at least one task (no orphan DS — validation checks)
   - \`spec_ref\` (required for behavior): the delta-spec domain
   - \`files\` (required): full relative paths from project root
   - \`acceptance\`: observable criteria
   - \`type\`: behavior/config/refactor/docs/scaffolding
4. Write the completed tasks to \`tasks.md\`
5. **Leave all checkboxes UNCHECKED** — the executor marks them after implementation
