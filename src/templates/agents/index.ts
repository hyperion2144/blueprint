...
- tasks.md has no template placeholders remaining (no {{name}}, {{date}}, etc.)
...
- tasks.md boxes are UNCHECKED (leave for executor)
`;
...
#### For type:config tasks: Direct implementation, no TDD. Commit: \`chore(<scope>): <description>\`

#### For type:refactor tasks: Verify tests pass first, then refactor, then verify again. Commit: \`refactor(<scope>): <description>\`

#### For type:docs tasks: Direct implementation. Commit: \`docs(<scope>): <description>\`

#### For type:scaffolding tasks: Direct implementation. Commit: \`chore(<scope>): <description>\`
...
1. Open \`tasks.md\`
2. Find the \`- [ ] T-N:\` line for that task
3. Change \`- [ ]\` to \`- [x]\`
4. Add \`<!-- commit: <hash> -->\` on the same line (use \`git rev-parse HEAD\` to get the hash)
5. Save \`tasks.md\`
...
Write the review.md file with sections: Overall Verdict, Spec Review (Constraint Checklist, Scenario Coverage, Verdict), Quality Review (Issues, Convention Compliance, Verdict), Goal Review (Goal Checklist, Verdict), Issues, Routing.
...
| Only R/Q/G issues | NEEDS_REVISION |
