/**
 * codex/handler.test.ts — Codex handler runtime tests
 *
 * T-4 RED: GIVEN a generated handler and a hook event payload
 *          WHEN the handler processes each supported event
 *          THEN it emits the expected bp-context or bp-workflow-state
 *               payload and exits successfully for disabled/missing-config
 *               cases.
 */
...
describe('Codex handler runtime helpers', () => {
  ...
  describe('hasBpConfig', () => {
    ...
    beforeEach(() => {
      testDir = mkdtempSync(join(tmpdir(), 'bp-codex-handler-'));
    });
  ...
  describe('generateWorkflowState', () => {
    it('returns a fallback when bp/state.md is absent', () => {
      const cwd = mkdtempSync(join(tmpdir(), 'bp-codex-state-'));
      try {
  ...
  describe('dispatchHandler event mapping', () => {
    ...
    beforeEach(() => {
      cwd = mkdtempSync(join(tmpdir(), 'bp-codex-dispatch-'));
      writeConfig(cwd, 'bp/config.yaml', 'version: 2\n');
      process.env.BP_HOOKS = undefined;
      process.env.BP_DISABLE_HOOKS = undefined;
    });
...
describe('generateCodexHandler', () => {
  it('returns the .codex/hooks/bp-handler.mjs descriptor', () => {
    const files = generateCodexHandler({} as ProjectConfig);
    expect(files).toHaveLength(1);
    expect(files[0].path).toBe(CODEX_HANDLER_PATH);
  ...
  it('is deterministic — two consecutive calls produce byte-identical output', () => {
    const a = generateCodexHandler({} as ProjectConfig);
    const b = generateCodexHandler({} as ProjectConfig);
    expect(a[0].path).toBe(b[0].path);
    expect(a[0].content).toBe(b[0].content);
  });
});

describe('HANDLER_SOURCE byte-determinism', () => {
  ...
  it('contains no Date.now() / Math.random() calls', () => {
    // Determinism: the source must not introduce runtime timestamps/randomness.
    expect(HANDLER_SOURCE).not.toMatch(/Date\.now\(/);
    expect(HANDLER_SOURCE).not.toMatch(/Math\.random\(/);
  });
});