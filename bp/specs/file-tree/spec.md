<!-- BOOTSTRAPPED — extracted from src/core/file-tree.ts, src/commands/blueprint-archive.ts, src/commands/blueprint-change.ts, src/commands/blueprint-list.ts, src/commands/blueprint-template.ts -->

## Purpose

Manage the blueprint directory structure — creating, listing, and archiving milestones, phases, and changes. Provide artifact template generation and change lifecycle management.

### Requirement: Directory Skeleton

The system SHALL create the blueprint directory skeleton during initialization with these directories: `specs/`, `conventions/`, `changes/`, `archive/`, `research/`, `templates/`, `milestones/`.
...
- **WHEN** `createSpecwfStructure("/project/blueprint")` is called
- **THEN** all 7 directories SHALL be created

### Requirement: Initialization Check

The system SHALL provide `isInitialized(blueprintDir)` returning `true` only if `state.md` exists in the blueprint directory.

- **Source:** `src/core/file-tree.ts:29-32` `isInitialized()`  
- **Confidence:** HIGH

#### Scenario: Already initialized
- **GIVEN** `blueprint/state.md` exists
- **WHEN** `isInitialized()` is called
- **THEN** `true` SHALL be returned

#### Scenario: Not initialized
- **GIVEN** `blueprint/` directory does not exist
- **WHEN** `isInitialized()` is called
...
- **THEN** `blueprint/milestones/m1/` SHALL be created and its path returned

#### Scenario: Create phase directory
- **GIVEN** milestone `m1` exists
- **WHEN** `createPhaseDir(dir, "m1", "auth")` is called
- **THEN** `blueprint/milestones/m1/auth/` SHALL be created
...
- **THEN** `blueprint/changes/login-flow/` SHALL be created

#### Scenario: Create adhoc change directory
- **GIVEN** a blueprint directory
- **WHEN** `createAdhocChangeDir(dir, "hotfix")` is called
- **THEN** `blueprint/changes/hotfix/` SHALL be created

### Requirement: Change Initialization

When creating a new change via `blueprint change new <name>`:
...
- **Source:** `src/commands/blueprint-change.ts:25-81` `newChange()`  
- **Confidence:** HIGH

#### Scenario: Fast path change creation
- **GIVEN** `blueprint change new feature-x` is invoked
...
- **GIVEN** `blueprint change new feature-y --full` is invoked
- **WHEN** the handler executes
...
- **Source:** `src/commands/blueprint-template.ts`, `src/templates/artifacts/index.ts`  
- **Confidence:** HIGH

#### Scenario: Generate proposal template
- **GIVEN** `blueprint template proposal --name my-change` is invoked
- **WHEN** the handler executes
- **THEN** `blueprint/changes/my-change/proposal.md` SHALL be created with `{{name}}` replaced by `my-change`

#### Scenario: Unknown template type
- **GIVEN** `blueprint template nonexistent` is invoked
- **WHEN** the handler executes
- **THEN** the CLI SHALL exit with code 1 and list available types

### Requirement: Change Listing

The `blueprint list` command SHALL display the hierarchy: milestones → phases → changes, plus adhoc changes, and optionally archived items.

- **Source:** `src/commands/blueprint-list.ts`  
- **Confidence:** HIGH

#### Scenario: List with items
- **GIVEN** milestones, phases, and changes exist
- **WHEN** `blueprint list` is invoked
...
- **WHEN** `blueprint list --all` is invoked
