...
项目调研（项目级技术选型）、codebase 映射（存量代码分析）和阶段调研（实现方案调研）是三个不同的职责，但之前共用同一个 `specwf-researcher` agent。缺少专门的阶段调研 agent 和对应的输出模板。
...
| `specwf-codebase-mapper` | 存量代码分析 | codebase/map | `specwf init --brownfield` |
...
| `src/commands/specwf-template.ts` | 修改 | `TEMPLATE_TYPES` 注册 `phase-research` → `research/phase-research.md` |
...
| `src/public/templates/commands/research-phase.md` | 修改 | 命令描述从 `specwf-researcher` 改为 `specwf-phase-researcher`，新增 `specwf template phase-research` 使用说明 |
