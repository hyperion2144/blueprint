...
| src/commands/blueprint-archive.ts | 修改 | archive 时检查 summary.md |
| src/commands/blueprint-template.ts | 修改 | 注册 change-summary 类型 + 修复 --dir 文件名 bug |

## 关键决策

- Change 级 summary 独立于 Phase 级 summary，内容更精简（产出文件 + 决策 + 验证）
- executor agent 完成后执行 `blueprint template change-summary` 获取模板
