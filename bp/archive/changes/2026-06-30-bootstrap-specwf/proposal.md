# Proposal: Bootstrap specwf Core

## Intent

specwf 是一个规格驱动开发工作流系统。本项目是 specwf 的自举——用 specwf 自身的流程来构建 specwf CLI。此 change 定义 specwf 的完整架构设计，作为后续所有实现工作的规格基础。
...
```
用户: /specwf:plan add-auth
  ↓ slash command 扩展为 prompt
prompt body:
  "读取 skill://specwf-plan 获取设计工作流指引。
   运行 `specwf context plan` 获取上下文清单（spec 注入）。
   按工作流执行，每步通过 bash 调用 specwf CLI 管理产物。
...
| 层 | OMP 机制 | 位置 | 角色 |
|---|---|---|---|
| CLI | 外部二进制，bash 调用 | `specwf <cmd>` | 产物管理（init/state/archive/config/context） |
...
`specwf update` 命令读取 `project.yml` 的 platform 配置，为每个平台生成对应的 slash command + skill + agent 文件。

### 8. CLI 命令

| 命令 | 作用 |
|---|---|
| `specwf init` | 初始化 specwf/ 目录结构 + 生成平台文件 |
| `specwf update` | 重新生成平台命令文件 |
| `specwf config [set]` | 查看/修改配置 |
| `specwf state` | 查看当前状态 |
| `specwf context <step>` | 输出步骤上下文清单（spec 注入） |
| `specwf continue` | 状态机推进 + 输出下一步 |
| `specwf archive <change>` | 归档（delta 合并 + 代码提取回灌） |
...
| 命令 | 步骤 | 子代理 |
|---|---|---|
| `/specwf:init` | 项目初始化 | ✓ codebase+spec 并行（存量） |
| `/specwf:grill` | 需求探讨 | - |
| `/specwf:research` | 项目调研 | ✓ 并行多方向 |
| `/specwf:roadmap` | 路线图 | - |
| `/specwf:discuss` | Phase 讨论 | - |
| `/specwf:research-phase` | Phase 调研 | ✓ 并行 |
| `/specwf:split` | Change 拆分 | - |
| `/specwf:plan` | Change 设计 | ✓ 可并行 |
| `/specwf:apply` | 实现 | ✓ 分组并发 |
| `/specwf:review` | 审查 | ✓ 三重并行 |
| `/specwf:verify` | 验证 | ✓ 可并行 |
| `/specwf:archive` | 归档 | - |
| `/specwf:ship` | 交付 | - |
...
|---|---|---|---|---|
| specwf-researcher | 技术调研 → STACK/ARCH/PITFALLS/RESEARCH | read,grep,glob,lsp,web_search,write,bash | research | high |
| specwf-planner | Change 设计 → proposal/design/tasks/delta-specs | read,grep,glob,lsp,write,bash | plan | high |
| specwf-executor | 代码实现 → TDD RED→GREEN→REFACTOR | read,edit,write,bash,grep,glob,lsp,ast_grep,ast_edit | execute | medium |
| specwf-reviewer | 三重审查（batch×3 并行）→ review 报告 | read,grep,glob,lsp,ast_grep,bash | review | high |
| specwf-verifier | 测试验证 → 诊断路由回环 | read,bash,grep,glob,lsp,edit,write | verify | medium |
| specwf-archiver | 归档 → delta 合并 + 代码认知回灌 | read,grep,glob,write,bash,edit | archive | - |

执行模式：子代理直接写文件到 `specwf/` 目录，通过 bash 调用 `specwf` CLI 管理状态转换。`specwf update` 根据 `project.yml` 的 `models` 配置生成 agent 定义文件。
...
`/specwf:continue` 读 state.md → 状态机确定当前位置+下一步 → 自动触发对应 slash command。
...
根级 `specwf/changes/<name>/`，与阶段无关。可选 proposal 步骤，然后直接走 Change 循环。archive 统一到 `specwf/archive/`。

### 21. 多平台

CLI 生成平台特定文件。`specwf update` 检测 `project.yml` 的 platform 配置，为每个平台生成对应 slash commands + skills + agent 定义。先 OMP（`.omp/commands/` + `.omp/agents/` + `skills/`），后 Claude Code（`.claude/commands/` + `.claude/agents/`）。
