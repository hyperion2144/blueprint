...
## Intent
Refactor blueprint template architecture: unify command/skill generation from TypeScript source, switch all content to English, make `blueprint continue` output inline instructions, and follow OpenSpec's template format.
...
| `src/commands/blueprint-template.ts` | Modify — read from ARTIFACT_TEMPLATES registry |
| `src/commands/blueprint-change.ts` | Modify — read templates from TS, not disk |
| `src/commands/blueprint-continue.ts` | Modify — inline instructions output + continueChangeHandler state fix |
...
|| `tests/integration/blueprint.test.ts` | Modify — same |
|| `src/commands/blueprint-archive.ts` | Modify — fix summary.md → change-summary.md filename |
|| `README.md` | Modify — updated to reflect current architecture, commands, version |
|| `blueprint/project.md` | Modify — updated to reflect template architecture, state machine, v0.2.2 |
...
- **Inline instructions**: `blueprint continue` outputs full step instructions — agents execute without reading a second file
...
- **Two continue commands**: `blueprint continue` (project/phase) vs `blueprint continue change <name>` (change) clearly separated

## Bugs Fixed

| Bug | Root Cause | Fix |
|-----|-----------|-----|
| `blueprint continue change <name>` never advances state | `continueChangeHandler` had no `updateState` call | Added state update with short-status derivation |
...
- CLI: `blueprint update` regenerates 40 files correctly
