...
影响所有使用 blueprint 的用户——每次 ship 都要手动补这些内容，或者像刚才一样产出薄薄的 PR。

本变更为 **feature enhancement**，目标是让 `blueprint ship`（或等效的自动化流程）能自动生成完整的交付内容。
...
- 新增 `blueprint ship` 子命令，自动执行 Phase ship 和 Milestone ship 的文档生成部分
- `blueprint ship phase <phase-name>` — 自动生成 PR body（含 change 列表、产出文件、测试结果、checklist）
- `blueprint ship milestone` — 自动生成 release notes（含 diff 统计、版本对比、breaking changes）
...
新增 `src/commands/blueprint-ship.ts` 命令文件，提供两个子命令：

```
blueprint ship phase [--phase <name>]
blueprint ship milestone [--milestone <name>]
```

**数据源**：
- `blueprint/state.md` — 获取当前 milestone/phase 状态和 change 列表
- `blueprint/milestones/<name>/phases/<phase>/summary.md` — 每个 phase 的完成摘要
- `blueprint/changes/<change>/summary.md` — 每个 change 的产出明细、验证结果
- `blueprint/changes/<change>/design.md` — 关键设计决策
...
- `src/commands/blueprint-ship.ts` — commander 注册

---

## Must-haves

- `blueprint ship phase` 输出的 PR body 必须包含：change 列表（含每个 change 的 scope 摘要）、测试结果（pass/fail + 数量）、构建状态
- `blueprint ship milestone` 输出的 release notes 必须包含：版本号、变更总览、breaking changes（如有）、git diff 统计（+/- 行数、文件数）
