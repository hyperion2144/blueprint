...
specwf 的三层架构（CLI + Slash Commands + Skills + 子代理 Agent）需要在 `specwf update` 时生
成 OMP 平台文件。本文档覆盖：

- `.omp/commands/*.md`（slash command）的格式规范和生成方案
- `.omp/agents/*.md`（task 子代理）的格式规范和生成方案
- `skills/<name>/SKILL.md`（工作流指引）的格式规范和生成方案
- `specwf context <step>` 上下文注入机制设计
- 集成陷阱和注意事项

---

## 一、Slash Command 文件：`.omp/commands/specwf-<step>.md`
...
```yaml
---
name: specwf:plan          # 覆盖文件名，即 /specwf:plan
...
**模板引擎设计**（CLI 实现 `specwf update` 时内置）：
...
**slug 命名规则**：`specwf-<step>.md`，其中 `step` 为 `plan`、`review` 等。frontmatter
中 `name: specwf:<step>` 覆盖文件名。
...
## 1. 读取上下文
- 运行 `specwf context plan` 获取上下文清单
- 按清单逐个 `read` 文件

## 2. 读取 Skill 指引（可选）
- 用 `read skill://specwf-plan` 获取工作流详细指引

## 3. 执行步骤
- （子代理 fan-out 等高成本操作）
- 每步通过 bash 调用 `specwf` CLI 管理产物/状态
...
```
1. 读取上下文（specwf context <step>）
2. 读取 skill 指引（read skill://specwf-<step>）
3. 执行工作流核心逻辑
4. 使用 task 工具 fan-out 子代理
5. 通过 bash 调用 specwf CLI 管理产物/状态
```
...
| 文件名 | 步骤 | 子代理 |
|---|---|---|
| `specwf-init.md` | init | research (codebase+spec 并行) |
| `specwf-grill.md` | grill | - |
| `specwf-research.md` | research | 并行多方向 |
| `specwf-roadmap.md` | roadmap | - |
| `specwf-discuss.md` | discuss | - |
| `specwf-research-phase.md` | research-phase | 并行 |
| `specwf-split.md` | split | - |
| `specwf-plan.md` | plan | 可并行 |
| `specwf-apply.md` | apply | 分组并发 |
| `specwf-review.md` | review | 三重并行 |
| `specwf-verify.md` | verify | 可并行 |
| `specwf-archive.md` | archive | - |
| `specwf-ship.md` | ship | - |
| `specwf-continue.md` | continue | - |

**`specwf-continue.md` 特殊处理**：body 是"读 state.md → 状态机确定下一步 → 对应 slash command"
不需要子代理。
...
\`\`\`
specwf context plan $1
\`\`\`

按输出的文件路径清单逐个读取。

## 2. 读取工作流指引
用 `read skill://specwf-plan` 获取详细设计流程指引。
...
## 5. 产物管理
通过 bash 调用 specwf CLI：
- 状态管理：\`specwf state\`
- 文件模板：\`specwf template design\`
...
## 1. 获取上下文
\`\`\`
specwf context ${def.step} $1
\`\`\`

## 2. 读取工作流指引
用 \`read skill://specwf-${def.step}\` 获取详细指引。

## 3. 执行工作流
${stepSpecificBody(def.step)}

## 4. 产物管理
通过 \`bash\` 调用 specwf CLI 管理产物和状态。
...
## 二、Agent 文件：`.omp/agents/specwf-<role>.md`
...
| 字段 | OMP 行为 | specwf 策略 |
|---|---|---|
| `name` | required，必须唯一 | `specwf-<role>`（researcher/planner/executor/reviewer/verifier/archiver） |
...
```markdown
你是一个 specwf 的 ${role} 子代理。

## 核心约束
- 所有产物写入 specwf/ 目录
- 通过 bash 调用 specwf CLI 管理状态
...
systemPrompt 中**不包含**具体项目上下文（那由 `specwf context <step>` 输出）。Agent 通
过 slash command 的 prompt 模板获取「运行 specwf context <step>」的指令。
...
- `specwf init` 时首次生成全部 6 个 agent 文件
- `specwf update` 时重新生成（覆盖已有文件）
- 当 `project.yml` 的 `models` 或 `profile` 变更时也需要 update

---

## 三、Skill 文件：`skills/specwf-<step>/SKILL.md`
...
# 以下可选
globs: ["specwf/changes/**/design.md"]
...
## 参考
- 子代理: specwf-researcher / specwf-planner
- 配置文件: \`specwf context plan\`
```
...
| Skill 名 | 对应步骤 | 核心指引内容 |
|---|---|---|
| `specwf-init` | init | codebase mapping + spec bootstrap + 目录初始化 |
| `specwf-grill` | grill | 需求探讨流程（21 项决策模版） |
| `specwf-research` | research | 项目级技术调研流程（多方向并行） |
| `specwf-roadmap` | roadmap | Milestone×M → Phase×N 路线图 |
| `specwf-discuss` | discuss | Phase 讨论：context.md 产出 |
| `specwf-research-phase` | research-phase | Phase 技术调研 |
| `specwf-split` | split | Change 依赖图拆分 |
| `specwf-plan` | plan | Change 设计：design+tasks+delta-specs |
| `specwf-apply` | apply | 代码实现：TDD+分组并发 |
| `specwf-review` | review | 三重审查：批量 tasks[3] |
| `specwf-verify` | verify | 测试验证 + 诊断路由 |
| `specwf-archive` | archive | delta 合并 + 代码认知回灌 |
| `specwf-ship` | ship | Phase/Milestone 交付 |
| `specwf-continue` | continue | 状态机推进流程 |
...
## 四、Context 注入机制：`specwf context <step>`

### 4.1 设计目标

`specwf context <step>` 是 spec 注入的核心入口。agent 在 slash command 中被指引「运行
`specwf context <step>` 获取上下文清单」，然后逐个 `read` 输出中的文件路径。
...
`specwf context` 需要知道哪些 specs 是"相关的"。判定规则：
...
`specwf context` 不操作 state.md（只读）。`specwf continue` 负责状态转换。

`specwf context` 读取 state.md 确定"当前在哪"，但不写入任何状态。
...
（比如其他项目的 `specwf-executor`），当前项目的 agent 可能被 shadow。

**缓解策略**：
- 使用 `specwf-` 前缀降低冲突概率
- 在 `specwf update` 输出中提示"生成 agent 文件：若与已有文件冲突，项目级
  文件优先"
...
的工具集中移除（而不是报错）。对 specwf 的三层嵌套（slash command → agent →
sub-agent → sub-sub-agent），如果在 strict profile 中配置了 low maxRecursionDepth，
可能导致深度不足。

**缓解**：在 `specwf` 文档中说明 agent 深度需求（3 层），引导用户配置
...
13 个 skill 都必须平铺在 `skills/specwf-<step>/SKILL.md` 模式下。不能为了
组织性放子目录。

**T11: Body 不自动注入**

Skill 文件是"被动"的——agent 必须显式调用 `read skill://<name>` 读取。
slash command body 中应包含 `read skill://specwf-<step>` 的指令。不要假设
skill 内容自动出现在 agent 上下文中。

**T12: 同名的 skill 覆盖**

如果用户的项目中已经有同名 skill（如其他插件提供的 `specwf-plan`），项目级
skill 按优先级规则覆盖。但如果用户手动创建了 `~/.omp/agent/skills/specwf-plan/`
...
无法通过 frontmatter 覆盖。specwf 的 slash command 不需要这个功能，所以
无关紧要。
...
运行子代理。只要 specwf CLI 和 agent 在同一个项目根启动，相对路径是安全的。
...
对于大型项目，`specwf context plan` 可能需要遍历 specs/ 目录查找相关文件。
...
在依赖图并行执行 change 时，B 依赖 A。A 还没完成时 `specwf context plan B`
不应该引用 A 的产出（因为还不存在）。context 命令应该检查 state.md 中依赖
change 的状态，只输出已完成的部分。

### 5.5 `specwf update` 通用陷阱

**T18: 多次运行覆盖用户修改**

`specwf update` 会覆盖 `.omp/commands/` 和 `.omp/agents/` 和 `skills/` 下
specwf 生成的文件。如果用户手动修改了生成的文件，下次 update 会丢失修改。

**缓解方案**：
1. 文件头添加注释标记 `# Generated by specwf update — do not edit by hand`
...
interface GeneratedFile {
  path: string;                    // 目标路径，如 .omp/commands/specwf-plan.md
  content: string;
  platform: string;
}
...
└── generators/
    ...
    ├── context.ts            # specwf context <step> 逻辑
...
### Slash Command: `.omp/commands/specwf-plan.md`
...
## 2. 读取工作流指引
用 \`read skill://specwf-plan\` 获取详细设计流程。
...
## 4. 产物管理
通过 bash 调用 specwf CLI 管理产物和状态转换。

## 5. 子代理
必要时用 task 工具 fan-out 子代理（specwf-researcher/specwf-planner）。
```

### Agent: `.omp/agents/specwf-researcher.md`
