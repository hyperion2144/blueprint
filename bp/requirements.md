...
构建 specwf — 一个独立 TypeScript CLI 包，为 AI 编码 agent 提供规格驱动的开发工作流。融合 OpenSpec（CLI 架构/change 结构）、GSD Core（milestone/phase/并行执行）、Trellis（spec 注入/代码认知回灌）的核心能力。
...
6. **产物目录**: 嵌套式 `specwf/`（非隐藏），change 放在 `milestones/<ms>/phases/<ph>/changes/<name>/` 下
7. **临时 change**: 根级 `specwf/changes/<name>/`，与阶段无关，直接走 Change 循环
...
10. **spec 自动注入**: `specwf context <step>` 输出当前步骤上下文清单（相关 specs + conventions），agent 按清单读取
...
20. **continue**: `/specwf:continue` 读 state.md，状态机确定下一步，自动触发对应 slash command
...
| `specwf update` | 重新生成平台命令文件（slash commands + skills + agents） |
...
|---|---|---|---|---|
| specwf-researcher | 技术调研 | read,grep,glob,lsp,web_search,write,bash | research | high |
| specwf-planner | Change 设计 | read,grep,glob,lsp,write,bash | plan | high |
| specwf-executor | 代码实现 | read,edit,write,bash,grep,glob,lsp,ast_grep,ast_edit | execute | medium |
| specwf-reviewer | 三重审查 | read,grep,glob,lsp,ast_grep,bash | review | high |
| specwf-verifier | 测试验证 | read,bash,grep,glob,lsp,edit,write | verify | medium |
| specwf-archiver | 归档 | read,grep,glob,write,bash,edit | archive | - |
